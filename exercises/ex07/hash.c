...
{
  //Save both values and return the comparision
  int* i = (int*) ip;
  int* j = (int*) jp;
  if (*i == *j){
    return 1;
...
int equal_string (void *s1, void *s2)
{
  ...
  char* str1 = (char*) s1;
  char* str2 = (char*) s2;
  //Compare them and return a boolean result
  if (strcmp(str1, str2)){
    return 0; 
  } else {
    return 1;
...
{
  //Use the correct equal function to compare the keys
...
//Returns index of node in a map
...
{
  //Save the index of the key in the map
  int i = map_index(map, key);
  //Prepend the new values onto the map
